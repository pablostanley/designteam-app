...
- [x] `designteam run <plan-id> <task-id>` — shipped in PR #30, extended in PR #37. Full end-to-end: atomic checkout → adapter dispatch → status transition → activity log emission. Resolution order: `opts.adapter` (BYO) → `--adapter=<id>` (registry lookup) → `--command=<shell>` (ephemeral local-script). Built-in `@designteam/adapter-claude-cli` auto-registered on CLI startup via `cli/builtin-adapters.mjs`. Unknown `--adapter` IDs fail fast without claiming the task. Error outcome releases the lock; terminal outcomes auto-release via `setTaskStatus`.
...
- [x] **Auto-publish on version tag** — shipped in PR #39 (workflow ready; NEEDS `NPM_TOKEN` secret set on the repo before first tag push). `.github/workflows/publish.yml` triggered by `v*.*.*` tags + manual dispatch. Invokes `scripts/publish-if-changed.sh <package>` for each workspace package in dependency order (core → adapter-utils → adapter-local-script + adapter-claude-cli → designteam CLI). Script compares each package's on-disk version to npm's latest and skips cleanly when they match — partial-failure re-triggers don't crash. Uses `--access public` for scoped packages and `--provenance` via `id-token: write`.
- [x] **Publish-readiness audit + runbook** — shipped in PR #38. Every workspace package passes `npm pack --dry-run` with only the expected files (dist, README, package.json). `@designteam/adapter-utils` gained a README (was missing). New `doc/PUBLISHING.md` documents the publish order (core → adapter-utils → adapter-local-script + adapter-claude-cli → designteam CLI), per-package checklist, and the exact commands. Three adapter packages ready for their first publish.
...
- [x] **`designteam adapters` CLI command** — shipped in PR #45. Lists every adapter the CLI can resolve via `--adapter=<id>`: built-ins (claude-cli always, anthropic-api when `ANTHROPIC_API_KEY` is set, efecto when `EFECTO_API_KEY` is set) plus any third-party adapter that self-registers. Covers the common "what are my options?" question without `grep`ing cli/builtin-adapters.mjs. New `adapters` eval scenario verifies baseline + env-gated registration.
- [x] **`designteam run --dry-run`** — shipped in PR #50. Preview mode: resolves plan/task/agent/adapter and prints what a real dispatch would hit (including budget status with OK/WARN/OVER flag), but claims no checkout, emits no activity, writes no ledger entry. Unknown adapter ids are flagged as "NOT REGISTERED" instead of exploding. New `run-dry-run` eval asserts side-effect-free preview via a sentinel file the command would create if it ran.
- [x] **`designteam doctor`** — shipped in PR #52. One-shot setup/health check. Seven rows with ✓/⚠/✗ glyphs: team.json, `.designteam/` writability, registered adapters, `ANTHROPIC_API_KEY`, `EFECTO_API_KEY`, Supabase env (cloud sync), budget cap state, stranded tasks (30m threshold). Exits 1 on any ✗ so CI / smoke tests can gate on it. New `doctor` eval covers empty sandbox, post-install, and API-key-flip paths.
...
- [x] **Heartbeats + stranded-work recovery** — shipped in PR #34. Single-runner variant: `designteam recover <plan> [--stale-minutes=N] [--dry-run]`. Scans for `in_progress` tasks whose `updatedAt` is older than the threshold (default 30m), reports them oldest-first with the prior `checkoutId`, and resets them to `todo` with the checkout released. `--dry-run` previews without mutating. `task.recovered` activity event carries the prior checkoutId + threshold for forensics. Skips still-live tasks whose `updatedAt` is recent. No daemon needed — invoked on demand by human, cron, or Claude Code. Paperclip's recovery-wake-then-blocked escalation deferred until we have a scheduler to do it.

### Safety + governance (before `designteam run` goes autonomous)

- [x] **Budget hard-stop** — shipped in PR #32. `.designteam/budget.json` (cap + period) + `.designteam/budget.jsonl` (append-only spend ledger). `designteam budget show | set --usd=N | reset`. Runner checks `isOverBudget()` before dispatch and refuses with the cap + current spend in the error message; after a successful run it calls `adapter.reportCost()` and appends to the ledger. Monthly UTC period auto-rolls via `resetPeriod()`. Soft 80% warning shipped in PR #41 — `getBudgetStatus()` returns `ok|warn|over|unset`; runner still dispatches at `warn` but prints `⚠️` stderr + emits `budget.warning` activity; `budget show` flips to `WARN` with remaining runway.
...
- [ ] **`packages/adapters/` implementations** — one package per runtime: `@designteam/adapter-claude-local`, `@designteam/adapter-codex-local`, `@designteam/adapter-cursor-local`, `@designteam/adapter-gemini-local`, `@designteam/adapter-efecto`. Each implements `TaskAdapter`.
  ...
  - [x] **`@designteam/adapter-efecto`** — shipped in PR #43 (V1). Creates an Efecto design session per task via `POST /api/v1/design/sessions` and returns the `designUrl` as the artifact, plus the agent's stitched brief as a second artifact so the next hop (human or tool-use loop) resumes with full Design Team context. 429 maps to `blocked` with `efecto.too_many_sessions` so a saturated IP doesn't fail the task. Auto-registers when `EFECTO_API_KEY` is set. V2 will add the autonomous LLM+MCP tool-use loop on top.
  - [x] **`@designteam/adapter-codex-local`** — shipped in PR #55. Wraps the local `codex` CLI via `codex exec <prompt>`. Under 60 LOC thanks to `buildAgentPrompt` + `runSubprocess` from adapter-utils — proof the extraction in PR #44 scales to new CLI runtimes. Serves as the template for the community cursor / gemini wrappers.
  - [ ] Remaining runtimes (cursor, gemini) — community-ownable. Template: copy `packages/adapter-codex-local`, swap the command + args pattern.
